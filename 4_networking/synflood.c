...
int main(int argc, char *argv[]) {
	...
	if ((context = libnet_init(LIBNET_RAW4, device, errbuf)) == NULL)
	{
		printf("Libnet Context Creation Error!");
		exit(1);
	}

	// dest_ip = libnet_name_resolve(argv[1], LIBNET_RESOLVE); // The host
	dest_ip = libnet_name2addr4(context, argv[1], LIBNET_RESOLVE); // The host
	dest_port = (u_short) atoi(argv[2]); // The port
	
	network = libnet_open_raw4(context); // Open network interface.
	...
	{
		libnet_build_ipv4(LIBNET_TCP_H, // Size of the packet sans IP header.
